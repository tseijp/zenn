...
WebGL ライブラリの Three.js が WebGPU 対応を進めており、Apple のサポートも進歩して実用性が高まってきました。WebGL は GLSL（OpenGL Shading Language）で記述しますが、WebGPU は WGSL（WebGPU Shading Language）で書きます。Three.js で WebGPU を扱う場合は独自の [TSL](https://github.com/mrdoob/three.js/wiki/Three.js-Shading-Language)（Three.js Shading Language）で記述するようです。
